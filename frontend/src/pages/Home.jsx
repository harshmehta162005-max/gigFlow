    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {gigs.map((gig) => (
            <div key={gig._id} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-6 hover:border-indigo-500/50 transition-all hover:transform hover:-translate-y-1 group relative">
              
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-xl font-bold text-white group-hover:text-indigo-400 transition-colors truncate pr-2">
                  {gig.title}
                </h3>
                <span className="bg-green-500/10 text-green-400 text-xs font-semibold px-2.5 py-0.5 rounded border border-green-500/20">
                  ${gig.budget}
                </span>
              </div>
              
              <p className="text-slate-400 text-sm mb-6 line-clamp-3 min-h-15">
                {gig.description}
              </p>
